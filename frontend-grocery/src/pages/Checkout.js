...
const Checkout = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Shape the order payload for the backend
    ...
    try {
      const data = await ordersAPI.create(orderItems, delivery, payment);
      // Expected: { success: true, data: { orderNumber, total, status, estimatedDelivery } }
      clearCart();
      navigate('/order-confirmation', {
        state: {
          orderNum: data.data.orderNumber,
      ...
      <div style={{ background: '#1e4d2b', padding: '32px 0', color: 'white', marginBottom: 32 }}>
        <div className="container">
          ...
          <div style={{ display: 'flex', gap: 12, color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem' }}>
            ...
            <span>›</span>
            <span style={{ color: 'white', fontWeight: 600 }}>Delivery & Payment</span>
            <span>›</span>
            ...
            <div className="col-lg-8">
              ...
              <div className="checkout-form-card">
                <h4 className="checkout-section-title">
                  <i className="bi bi-geo-alt me-2"></i>Delivery Information
                </h4>
                <div className="row g-3">
                  <div className="col-md-6">
                    <label style={{ fontSize: '0.82rem', fontWeight: 600, marginBottom: 5, display: 'block' }}>First Name *</label>
                    <input name="firstName" required className="form-control-custom" placeholder="John" value={form.firstName} onChange={change} />
                  </div>
                  <div className="col-md-6">
                    <label style={{ fontSize: '0.82rem', fontWeight: 600, marginBottom: 5, display: 'block' }}>Last Name *</label>
                    <input name="lastName" required className="form-control-custom" placeholder="Doe" value={form.lastName} onChange={change} />
                  </div>
                  <div className="col-md-6">
                    <label style={{ fontSize: '0.82rem', fontWeight: 600, marginBottom: 5, display: 'block' }}>Email Address *</label>
                    <input type="email" name="email" required className="form-control-custom" placeholder="john@example.com" value={form.email} onChange={change} />
                  </div>
                  <div className="col-md-6">
                    <label style={{ fontSize: '0.82rem', fontWeight: 600, marginBottom: 5, display: 'block' }}>Phone Number *</label>
                    <input name="phone" required className="form-control-custom" placeholder="+254 7XX XXX XXX" value={form.phone} onChange={change} />
                  </div>
                  <div className="col-12">
                    <label style={{ fontSize: '0.82rem', fontWeight: 600, marginBottom: 5, display: 'block' }}>Street Address *</label>
                    <input name="address" required className="form-control-custom" placeholder="123 Market Street, Apt 4B" value={form.address} onChange={change} />
                  </div>
                  <div className="col-md-6">
                    <label style={{ fontSize: '0.82rem', fontWeight: 600, marginBottom: 5, display: 'block' }}>City *</label>
                    <input name="city" required className="form-control-custom" placeholder="Nairobi" value={form.city} onChange={change} />
                  </div>
                  <div className="col-md-6">
                    <label style={{ fontSize: '0.82rem', fontWeight: 600, marginBottom: 5, display: 'block' }}>Postal Code</label>
                    <input name="zip" className="form-control-custom" placeholder="00100" value={form.zip} onChange={change} />
                  </div>
                </div>
              </div>
              ...
              <div className="checkout-form-card">
                ...
                {payment === 'card' && (
                  <div className="row g-3 mt-2">
                    <div className="col-12">
                      <input name="cardNum" className="form-control-custom" placeholder="Card number (e.g. 4111 1111 1111 1111)" value={form.cardNum} onChange={change} />
                    </div>
                    <div className="col-6">
                      <input name="cardExp" className="form-control-custom" placeholder="MM / YY" value={form.cardExp} onChange={change} />
                    </div>
                    <div className="col-6">
                      <input name="cardCvv" className="form-control-custom" placeholder="CVV" value={form.cardCvv} onChange={change} />
                    </div>
                  </div>
                )}
                {payment === 'mpesa' && (
                  <div className="mt-2">
                    <input name="phone" className="form-control-custom" placeholder="M-Pesa number (e.g. 0712 345 678)" value={form.phone} onChange={change} />
                    <p style={{ fontSize: '0.8rem', color: '#6c757d', marginTop: 6 }}>
                      You will receive a push notification to complete payment.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="order-summary-card">
                  ...
                  {items.map(item => (
                    <div key={item.id} style={{ display: 'flex', gap: 10, alignItems: 'center', paddingBottom: 12, marginBottom: 12, borderBottom: '1px solid #f0f0f0' }}>
                      ...
                      <div style={{ flex: 1, fontSize: '0.85rem' }}>
                        ...
                        <div style={{ color: '#6c757d' }}>×{item.quantity}</div>
                      </div>
                ...
                <button type="submit" className="place-order-btn" style={{ marginTop: 20 }} disabled={loading}>
                  {loading
                    ...
                    : <>Place Order · ${total.toFixed(2)} <i className="bi bi-check-circle ms-1"></i></>
                  }
                </button>

                <div style={{ marginTop: 16, padding: 14, background: '#e8f5e9', borderRadius: 10 }}>
                  <div style={{ display: 'flex', gap: 8, fontSize: '0.8rem', color: '#1a3d22' }}>
                    <i className="bi bi-shield-check" style={{ fontSize: 18 }}></i>
                    <span>Secure checkout. Your data is encrypted and safe.</span>
                  </div>
                </div>
              </div>
            </div>
